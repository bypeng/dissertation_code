...
main()
{
    //FILE                *fp_com; // For intermediate values
    FILE                *fp_req, *fp_rsp;
    int                 ret_val;
    int i;
    unsigned char *ct = 0;
    ...
    /* fp_com = fdopen(7, "w"); // For intermediate values
    if(!fp_com)
    ...
    */
    ...
    for (i=0; i<KATNUM; i++) {
        ...
        #ifdef INTERVALUE_GEN
          ivg_print_uint8array("pk", pk, crypto_kem_PUBLICKEYBYTES);
          ivg_print_uint8array("sk", sk, crypto_kem_SECRETKEYBYTES);
        #endif
        ...
        #ifdef INTERVALUE_GEN
          ivg_print_uint8array("ct", ct, crypto_kem_CIPHERTEXTBYTES);
          ivg_print_uint8array("ss", ss, crypto_kem_BYTES);
        #endif
