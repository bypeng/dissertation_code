...
ntt_modulename = 'ntt761_4591_by_114689_120833'
...
def NTTCheckFriendly():
  global p, p_cover, lg2_pc, q0, lg2_q0, lg2_2q0, qs, lg2_qs, lg2_2qs, qs_inv, qs_else, lg2_wmax, lg2_pqb, pqbound, lg2_pqb
  ...
  if(lg2_pc <= 5):
    print('This script only supports p > 16.')
    return False
  print('NTT{pc} is to be constructed, suitable for the ring F_{{{q0}}} [x] / \\langle x^{{{pc}}} - 1 \\rangle . '.format(pc=p_cover, q0=q0))

  if(not isprime(q0)):
    print('{} needs to be a prime number.'.format(q0))
    return False

  if((LeastOne(q0) >= lg2_pc) and (len(qs) != 0)):
    print('{} is actually friendly to NTT{}. No qs list necessary.'.format(q0, p_cover))
    return False

  if(LeastOne(q0) < lg2_pc):
    if(len(qs) == 0):
      print('{} is not friendly to NTT{}. Friendly prime list needs to be provided.'.format(q0, p_cover))
      return False
    else:
      ...
      for qi in qs:
        ...
        if(LeastOne(qi) < lg2_pc):
          print('It is found that {} is not friendly to NTT{}. Select another prime number.'.format(qi, p_cover))
          return False
...
def ntt_top_gen(fp):
  global ntt_modulename, p, p_cover, lg2_pc, q0, lg2_q0, lg2_2q0, qs, lg2_qs, lg2_2qs, qs_inv, qs_else, lg2_wmax, lg2_pqb
  ...
  print(    '  wire [{:<2d}: 0] data_index_rd [0 : 1];'.format(lg2_pc-2), file=fp)
  print(    '  wire [{:<2d}: 0] data_index_wr [0 : 1];'.format(lg2_pc-2), file=fp)
  ...
  print(    '  reg  [{:<2d} : 0] stage_bit;'.format(lg2_pc-2), file=fp)
  print(    '  wire [{:<2d} : 0] w_addr;'.format(lg2_pc-2), file=fp)
  print(    '', file=fp)
  ...
  lg2lg2_pc = int(math.ceil(math.log2(lg2_pc)))
  print(    '  reg  [2 : 0] state, next_state;', file=fp)
  print(    '  reg  [{:d} : 0] stage, stage_wr;'.format(lg2lg2_pc), file=fp)
  print(    '  wire [{:d} : 0] stage_rdM, stage_wrM;'.format(lg2lg2_pc), file=fp)
  print(    '  reg  [{:d} : 0] ctr;'.format(lg2_pc), file=fp)
  print(    '  reg  [{:d} : 0] ctr_shift_7, ctr_shift_8, ctr_shift_9, ctr_shift_1, ctr_shift_2;'.format(lg2_pc), file=fp)
  print(    '  reg          ctr_MSB_masked;', file=fp)
  print(    '  reg          poly_select;', file=fp)
  print(    '  reg          ctr_msb_shift_1;', file=fp)
  print(    '  wire         ctr_half_end, ctr_full_end, ctr_shift_7_end, stage_rd_end, stage_rd_2, stage_wr_end, ntt_end, point_proc_end, reduce_end;', file=fp)
  print(    '', file=fp)

  print(    '  // w_array', file=fp)
  print(    '  reg         [{:<2d}: 0] w_addr_in;'.format(lg2_pc-1), file=fp)
  if(len(qs) != 0):
  ...
  print(    '  reg [{:<2d}: 0] wr_ctr [0 : 1];'.format(lg2_pc-1), file=fp)
  print(    '  reg [{:<2d}: 0] din_shift_1, din_shift_2, din_shift_3;'.format(lg2_q0-1), file=fp)
  print(    '  reg [{:<2d}: 0] w_addr_in_shift_1;'.format(lg2_pc-1), file=fp)
  print(    '', file=fp)
  ...
  print(    '  addr_gen addr_rd_0 (clk, stage_rdM, {{ctr_MSB_masked, ctr[{:d}:0]}}, bank_index_rd[0], data_index_rd[0]);'.format(lg2_pc-2), file=fp)
  print(    '  addr_gen addr_rd_1 (clk, stage_rdM, {{1\'b1, ctr[{:d}:0]}}, bank_index_rd[1], data_index_rd[1]);'.format(lg2_pc-2), file=fp)
  ...
  print(    '  w_addr_gen w_addr_gen_0 (clk, stage_bit, ctr[{:d}:0], w_addr);'.format(lg2_pc-2), file=fp)
  print(    '', file=fp)

  print(      '  // Butterfly Unit ' + ('' if (len(qs) <= 1) else 's') + ' , each with a corresponding omega array', file=fp)
  if(len(qs) == 0):
    print(    '  bfu_{:d} bfu_inst (clk, ntt_state, in_a, in_b, in_w, bw, out_a, out_b);'.format(q0), file=fp)
    print(    '  w_{:d} rom_w_inst (clk, w_addr_in_shift_1, w_dout);'.format(q0), file=fp) 
  elif(len(qs) == 1):
    print(    '  bfu_{:d} bfu_inst (clk, ntt_state, in_a, in_b, in_w, bw, out_a, out_b);'.format(qs[0]), file=fp)
    print(    '  w_{:d} rom_w_inst (clk, w_addr_in_shift_1, w_dout);'.format(qs[0]), file=fp) 
  else:
    for idx, (qi, lg2_qi) in enumerate(zip(qs, lg2_qs)):
      print(  '  bfu_{q:d} bfu_inst{idx} (clk, ntt_state, in_a[{idx}], in_b[{idx}], in_w[{idx}], bw[{idx}], out_a[{idx}], out_b[{idx}]);'.format(q=qi, idx=idx), file=fp)
  ...
  print(    '  assign ctr_half_end         = (ctr[{:d}:0] == {:d}) ? 1 : 0;'.format(lg2_pc-2, (p_cover >> 1)-1), file=fp)
  print(    '  assign ctr_full_end         = (ctr[{:d}:0] == {:d}) ? 1 : 0;'.format(lg2_pc-1, p_cover-1), file=fp)
  # print(    '  assign ctr_shift_7_end = (ctr_shift_7[{:d} : 0] == {:d}) ? 1 : 0;'.format(lg2_pc-2, (p_cover >> 1)-1), file=fp)
  print(    '  assign stage_rd_end         = (stage == {:d}) ? 1 : 0;'.format(lg2_pc), file=fp)
  print(    '  assign stage_rd_2           = (stage == 2) ? 1 : 0;', file=fp)
  # print(    '  assign stage_wr_end    = (stage_wr == {:d}) ? 1 : 0;'.format(lg2_pc), file=fp)
  print(    '  assign ntt_end         = (stage_rd_end && ctr[{:d} : 0] == 10) ? 1 : 0;'.format(lg2_pc-2), file=fp)
  print(    '  assign crt_end         = (stage_rd_2 && ctr[{:d} : 0] == 10) ? 1 : 0;'.format(lg2_pc-2), file=fp)
  print(    '  assign point_proc_end   = (ctr == {:d}) ? 1 : 0;'.format(p_cover + 10), file=fp)
  print(    '  assign reload_end      = (stage != 0 && ctr[{:d}:0] == 4) ? 1 : 0;'.format(lg2_pc-2), file=fp)
  print(    '  assign reduce_end      = (ctr == {:d});'.format(p_cover+4), file=fp)
  print(    '', file=fp)
  ...
  print(    '        rd_addr[0][{:d}:0] <= data_index_rd[1];'.format(lg2_pc-2), file=fp)
  print(    '        rd_addr[1][{:d}:0] <= data_index_rd[0];'.format(lg2_pc-2), file=fp)
  print(    '      end else begin', file=fp)
  print(    '        rd_addr[0][{:d}:0] <= data_index_rd[0];'.format(lg2_pc-2), file=fp)
  print(    '        rd_addr[1][{:d}:0] <= data_index_rd[1];'.format(lg2_pc-2), file=fp)
  print(    '      end', file=fp)
  print(    '    end else begin', file=fp)
  print(    '      rd_addr[0][{ql:d}:0] <= data_index_rd[0];'.format(ql=lg2_pc-2), file=fp)
  print(    '      rd_addr[1][{ql:d}:0] <= data_index_rd[0];'.format(ql=lg2_pc-2), file=fp)
  print(    '    end', file=fp)
  print(    '', file=fp)
  ...
  print(    '    end else if (state == ST_RELOAD) begin', file=fp)
  print(    '      rd_addr[0][{:d}] <= 0;'.format(lg2_pc-1), file=fp)
  print(    '      rd_addr[1][{:d}] <= 0;'.format(lg2_pc-1), file=fp)
  print(    '    end else begin', file=fp)
  ...
  print(    '        wr_addr[0][{:d}:0] <= data_index_wr[1];'.format(lg2_pc-2), file=fp)
  print(    '        wr_addr[1][{:d}:0] <= data_index_wr[0];'.format(lg2_pc-2), file=fp)
  print(    '      end else begin', file=fp)
  print(    '        wr_addr[0][{:d}:0] <= data_index_wr[0];'.format(lg2_pc-2), file=fp)
  print(    '        wr_addr[1][{:d}:0] <= data_index_wr[1];'.format(lg2_pc-2), file=fp)
  print(    '      end', file=fp)
  print(    '    end else begin', file=fp)
  print(    '      wr_addr[0][{:d}:0] <= data_index_wr[0];'.format(lg2_pc-2), file=fp)
  print(    '      wr_addr[1][{:d}:0] <= data_index_wr[0];'.format(lg2_pc-2), file=fp)
  ...
  print(    '    end else if (state == ST_PMUL || state == ST_REDUCE || state == ST_FINISH) begin', file=fp)
  ...
  for idx, (qi, lg2_qi, lg2_qi_diff) in enumerate(zip(qsX, lg2_qsX, lg2_qsXdiff)):
    ...
    if(len(qs) == 1):
      ...
      print('    end else if (state == ST_CRT) begin', file=fp)
      print('      if (stage_wr == 0) begin', file=fp)
      print('        wr_din[0][{qh:d}:{ql:d}] <= out_a[{qid:d}];'.format(qh=lg2_q_st+lg2_qi-1, ql=lg2_q_st, qid=idx), file=fp)
      print('        wr_din[1][{qh:d}:{ql:d}] <= out_a[{qid:d}];'.format(qh=lg2_q_st+lg2_qi-1, ql=lg2_q_st, qid=idx), file=fp)
      print('      end else begin', file=fp)
      print('        wr_din[0][{qh:d}:{ql:d}] <= mod{q0:d}_out;'.format(qh=lg2_q_st+lg2_qi-1, ql=lg2_q_st, q0=q0), file=fp)
  ...
  print(      '    end else begin', file=fp)
  print(      '      w_addr_in <= {:d} - w_addr;'.format(p_cover), file=fp)
  print(      '    end', file=fp)
  print(      '', file=fp)
  ...
  print(    '      wr_ctr[0] <= addr[{:d}:0];'.format(lg2_pc-1), file=fp)
  ...
  for idx in range(lg2_pc):
    if (idx != 0): print(', ', end='', file=fp)
    print(         'ctr_shift_1[{:d}]'.format(idx), end='', file=fp)
  print(           '};', file=fp)
  print(    '    end else if (state == ST_NTT || state == ST_INTT) begin', file=fp)
  print(    '      wr_ctr[0] <= {{1\'b0, ctr_shift_7[{:d}:0]}};'.format(lg2_pc-2), file=fp)
  print(    '    end else begin', file=fp)
  print(    '      wr_ctr[0] <= ctr_shift_7[{:d}:0];'.format(lg2_pc-1), file=fp)
  print(    '    end', file=fp)
  print(    '', file=fp)
  print(    '    wr_ctr[1] <= {{1\'b1, ctr_shift_7[{:d}:0]}};'.format(lg2_pc-2), file=fp)
  print(    '  end', file=fp)
  print(    '', file=fp)
  ...
  print(    '    end else begin', file=fp)
  print(    '      ctr_MSB_masked = ctr[{:d}];'.format(lg2_pc-1), file=fp)
  print(    '    end', file=fp)
  print(    '  end', file=fp)
  print(    '', file=fp)

  print(    '  // ctr, ctr_shifts', file=fp)
  print(    '  always @ ( posedge clk ) begin', file=fp)
  print(    '    if (state == ST_NTT || state == ST_INTT) begin', file=fp)
  print(    '      if (ntt_end) begin', file=fp)
  print(    '        ctr <= 0;', file=fp)
  ...
  print(    '      end', file=fp)
  print(    '    end else if (state == ST_REDUCE) begin', file=fp)
  print(    '      if (reduce_end) begin', file=fp)
  print(    '        ctr <= 0;', file=fp)
  print(    '      end else begin', file=fp)
  print(    '        ctr <= ctr + 1;', file=fp)
  print(    '      end', file=fp)
  ...
  print(    '      end else if (ctr_shift_7[{:d}:0] == 0 && stage != 0) begin'.format(lg2_pc-2), file=fp)
  ...
  print(    '      if (reload_end) begin', file=fp)
  print(    '        stage_wr <= 0;', file=fp)
  print(    '      end else if (ctr_shift_7[{:d}:0] == 0 && stage != 0) begin'.format(lg2_pc-1), file=fp) # NOTE: evaluate delay
  print(    '        stage_wr <= stage_wr + 1;', file=fp)
  print(    '      end else begin', file=fp)
  print(    '        stage_wr <= stage_wr;', file=fp)
  print(    '      end', file=fp)
  ...
  print(    '      if (crt_end) begin', file=fp)
  print(    '        stage_wr <= 0;', file=fp)
  print(    '      end else if (ctr_shift_9[{:d}:0] == 0 && stage != 0) begin'.format(lg2_pc-1), file=fp) # NOTE: evaluate delay
  print(    '        stage_wr <= stage_wr + 1;', file=fp)
  print(    '      end else begin', file=fp)
  print(    '        stage_wr <= stage_wr;', file=fp)
  print(    '      end', file=fp)
  ...
  print(    '        stage_bit[{sh:d} : 1] <= stage_bit[{shm:d} : 0];'.format(sh=lg2_pc-2, shm=lg2_pc-3), file=fp)
  print(    '      end else begin', file=fp)
  ...
  print(    '      if(ntt_end && poly_select == 1)', file=fp)
  print(    '        next_state = ST_PMUL;', file=fp)
  print(    '      else', file=fp)
  print(    '        next_state = ST_NTT;', file=fp)
  print(    '    end', file=fp)
  print(    '    ST_PMUL: begin', file=fp)
  print(    '      if (point_proc_end)', file=fp)
...
def bfu_gen(fp, qi, lg2_qi, lg2_2qi, lg2_wmax):
  ...
  print(  'module bfu_{} ( clk, state, in_a, in_b, w, bw, out_a, out_b );'.format(qi), file=fp)
  print(  '', file=fp)
  print(  '  input                      clk;', file=fp)
  ...
  print(  '  reg signed        [{:<2d} : 0] a, b;'.format(lg2_qi), file=fp)
  ...
  print(  '', file=fp)
  print(  '  always @ ( posedge clk ) begin', file=fp)
  print(  '    a <= in_a_s4 + mod_bw;', file=fp)
  print(  '    b <= in_a_s4 - mod_bw;', file=fp)
  print(  '', file=fp)
...
def w_array_gen(fp, q, lg2_q):
  global p_cover, lg2_pc
  gen = find_gen(q, p_cover)
  ...
  print(    '  reg               [{:2d} : 0]  addr_reg;'.format(lg2_pc-1), file=fp)
  print(    '', file=fp)
  if(lg2_pc > 9):
    print(  '  (* rom_style = "block" *) reg signed [{:d}:0] data [0:{:d}];'.format(lg2_q-1,(p_cover>>1)-1), file=fp)
  else:
    print(  '  (* rom_style = "distributed" *) reg signed [{:d}:0] data [0:{:d}];'.format(lg2_q-1,(p_cover>>1)-1), file=fp)
  print(    '', file=fp)
  if(lg2_pc == 2):
    print(  '  assign dout_p = data[addr_reg[0]];', file=fp)
  else:
    print(  '  assign dout_p = data[addr_reg[{}:0]];'.format(lg2_pc-2), file=fp)
  print(    '  assign dout_n = -dout_p;', file=fp)
  print(    '  assign dout   = addr_reg[{}] ? dout_n : dout_p;'.format(lg2_pc-1), file=fp)
  print(    '', file=fp)
  print(    '  always @ ( posedge clk ) begin', file=fp)
  ...
  xlen = int(math.ceil(math.log10((p_cover>>1) + 1)))
  pstr =    '    data[{{:{:d}d}}] =  \'sd{{:d}};'.format(xlen)
  nstr =    '    data[{{:{:d}d}}] = -\'sd{{:d}};'.format(xlen)
  for idx in range(p_cover>>1):
    v = pow(gen, idx, q)
    if v > ((q-1) >> 1): v -= q
    if(v >= 0):
...
def NTT_RTLGen():
  global ntt_modulename, p, p_cover, lg2_pc, q0, lg2_q0, lg2_2q0, qs, lg2_qs, lg2_2qs, qs_inv, qs_else, lg2_wmax
  ...
  ntt_top_gen(fp)
  waddrgen_gen(fp, lg2_pc)
  addrgen_gen(fp, lg2_pc)
  bshift_gen(fp, lg2_pc)
