...
parser = argparse.ArgumentParser(description='Good Trick NTT with 5 pcs Test Data Generator')
...
parser.add_argument('-p', '--psize', metavar='p', type=int, required=True, default=1277, help='Polynomial size, larger than 20.')
parser.add_argument('-q0', '--primeQ', metavar='q0', type=int, required=True, default=7879, help='prime number Q.')
...
def __main__():
  ...
  if (len(qs) == 3):
    ...
    r_good = []
    for idx in range(goodfactor):
      r_good += [h_good_q1[idx][0][0]]
    h_q1 = GoodComposite(r_good, qs[0])
    h_q1 = R([ZZ(Fq1(h_q1[i])+qs_div2[0])-qs_div2[0] for i in range(pc2)])
    r_good = []
    for idx in range(goodfactor):
      r_good += [h_good_q2[idx][0][0]]
    h_q2 = GoodComposite(r_good, qs[1])
    h_q2 = R([ZZ(Fq2(h_q2[i])+qs_div2[1])-qs_div2[1] for i in range(pc2)])
    r_good = []
    for idx in range(goodfactor):
      r_good += [h_good_q3[idx][0][0]]
    h_q3 = GoodComposite(r_good, qs[2])
