...
lg2_2qs = [34, 35]
qs_inv = [15602, -23793]
qs_else = [(61, 12, 6), (5, 15, 3)]

lg2_wmax = 17

ntt_modulename = 'ntt1277good5_1277_by_163841_249857'
...
def NTTCheckFriendly():
  ...
  if(lg2_pgc <= 5):
    print('This script only supports p > 80.')
    return False
...
def ntt_top_gen(fp, mod_delay):
  ...
  print(    '  reg  [{:d} : 0] ctr_shift_7, ctr_shift_8, ctr_shift_9, ctr_shift_10, ctr_shift_1, ctr_shift_2, ctr_shift_3, ctr_pmul_shift;'.format(lg2_pgc), file=fp)
  ...
  if(goodfactor == 3):
    ...
    print(  '  // mod_5', file=fp)
    print(  '  wire [2 : 0] in_addr;', file=fp)
    print(  '  wire [2 : 0] out_good0, out_good1;', file=fp)
    print(  '  reg  [2 : 0] out_good0_0, out_good1_0;', file=fp)
    print(  '  wire         acc_ctrl;', file=fp)
    print(  '  reg  [9 : 0] acc_ctrls;', file=fp)
  print(    '', file=fp)
  ...
  if(goodfactor == 3):
    ...
    print(  '  mod_5 in_addr_gen ( clk, addr, in_addr );', file=fp)
    print(  '  good5_addr_gen good5_addr_0 ( clk, good_index, ctr_good, out_good0, out_good1, acc_ctrl );', file=fp)
    print(  '', file=fp)
  ...
  if(goodfactor == 3):
    ...
    print(  '  assign stage_rd_5           = (stage == 5) ? 1 : 0;', file=fp)
    print(  '  assign stage_rd_13          = (stage == 13) ? 1 : 0;', file=fp)
  ...
  if(goodfactor == 3):
    ...
    print(  '  assign crt_end              = (stage_rd_13 && ctr[{:d} : 0] == 10) ? 1 : 0;'.format(lg2_pgc-2), file=fp)
    print(  '  assign point_proc_end       = (ctr == {:d}) ? 1 : 0;'.format(p_g_cover + 2), file=fp)
    print(  '  assign reload_end           = (stage_rd_5 && ctr[{:d}:0] == 4) ? 1 : 0;'.format(lg2_pgc-2), file=fp)
  ...
  if(goodfactor == 5):
    print(  '  assign good_index_buf = { good_index[0], 1\'b1, !good_index[2], good_index[1] };', file=fp)
  print(    '', file=fp)
  ...
  if(goodfactor == 3):
    ...
    print(  '      rd_addr[0][{qh:d}:{ql:d}] <= {{ 1\'b1, !good_index[2], good_index[1] }};'.format(qh=lg2_pc-2, ql=lg2_pgc-1), file=fp)
    print(  '      rd_addr[1][{qh:d}:{ql:d}] <= {{ 1\'b1, !good_index[2], good_index[1] }};'.format(qh=lg2_pc-2, ql=lg2_pgc-1), file=fp)
  print(    '    end else begin', file=fp)
  ...
  print(    '    end else if (state == ST_RELOAD) begin', file=fp)
  if(goodfactor == 3 or goodfactor == 5):
  ...
  if(goodfactor == 3):
    ...
    print(  '      if ((ctr < 3) || (ctr_good != 0)) begin', file=fp)
  print(    '        wr_en[0] <= 0;', file=fp)
  print(    '        wr_en[1] <= 0;', file=fp)
  print(    '      end else begin', file=fp)
  ...
  for idx, (qi, lg2_qi, lg2_qi_diff) in enumerate(zip(qsX, lg2_qsX, lg2_qsXdiff)):
    ...
    if(len(qs) == 1):
      ...
      print('    end else if (state == ST_CRT) begin', file=fp)
      print('      if (stage_wr[{:d}] == 0) begin'.format(2 if (goodfactor == 3) else (3 if (goodfactor == 5) else 0)), file=fp)
      print('        wr_din[0][{qh:d}:{ql:d}] <= out_a[{qid:d}];'.format(qh=lg2_q_st+lg2_qi-1, ql=lg2_q_st, qid=idx), file=fp)
      print('        wr_din[1][{qh:d}:{ql:d}] <= out_a[{qid:d}];'.format(qh=lg2_q_st+lg2_qi-1, ql=lg2_q_st, qid=idx), file=fp)
      print('      end else begin', file=fp)
      print('        wr_din[0][{qh:d}:{ql:d}] <= mod{q0:d}_out;'.format(qh=lg2_q_st+lg2_qi-1, ql=lg2_q_st, q0=q0), file=fp)
  ...
  if ( len(qs) >= 2 ):
    ...
    if(goodfactor == 3):
      ...
      print(  '      if (stage[3] == 0 || (stage == 8 && ctr <= 3)) begin', file=fp)
    else:
  ...
  print(    '    end else if (state == ST_PMUL) begin', file=fp)
  if(goodfactor == 3 or goodfactor == 5):
    print(  '      wr_ctr[0] <= ctr_pmul_shift[{:d}:0];'.format(lg2_pgc-1), file=fp)
  print(    '    end else begin', file=fp)
  ...
  if(goodfactor == 3):
    print(  '    ctr_pmul_shift <= ctr_shift_1 - 2;', file=fp)
  elif(goodfactor == 5):
    print(  '    ctr_pmul_shift <= ctr - 2;', file=fp)
  print(    '  end', file=fp)
  print(    '', file=fp)
  ...
  if(goodfactor == 3):
    ...
    print(  '        stage <= 0;', file=fp)
    print(  '      end else if (ctr_full_end) begin', file=fp)
    print(  '        if(stage == 4) begin', file=fp)
    print(  '          stage <= 8;', file=fp)
    print(  '        end else begin', file=fp)
    print(  '          stage <= stage + 1;', file=fp)
    print(  '        end', file=fp)
    print(  '      end else begin', file=fp)
  ...
  if(goodfactor == 3):
    ...
    print(  '        stage_wr <= 0;', file=fp)
    print(  '      end else if (ctr_shift_9[{:d}:0] == 0 && stage != 0) begin'.format(lg2_pgc-1), file=fp)
    print(  '        if(stage_wr == 4) begin', file=fp)
    print(  '          stage_wr <= 8;', file=fp)
    print(  '        end else begin', file=fp)
    print(  '          stage_wr <= stage_wr + 1;', file=fp)
    print(  '        end', file=fp)
    print(  '      end else begin', file=fp)
...
# ----- mod_5 generator (TODO: Under modification!)
def mod5_gen(fp, lg2_pc):
  lg2_pc_it = lg2_pc
  r1bit = []
  lg2_r1 = []
  r2bit = []
  ...
  while(lg2_pc_it > 4):
    r2bit_it = lg2_pc_it // 2
    r2bit += [r2bit_it]
    lg2_r2_it = r2bit_it.bit_length()
    lg2_r2 += [lg2_r2_it]
    r1bit_it = r2bit_it + (1 if lg2_pc_it % 4 == 1 else 0)
    r1bit += [r1bit_it]
    lg2_r1_it = r1bit_it.bit_length()
    lg2_r1 += [lg2_r1_it]
    r4bit_it = (lg2_pc_it + 1) // 4
    r4bit += [r4bit_it]
    lg2_r4_it = r4bit_it.bit_length()
    lg2_r4 += [lg2_r4_it]
    summax_it = r1bit_it + 2 * r2bit_it + 4 * r4bit_it
    summax += [summax_it]
    lg2_pc_it = summax_it.bit_length()
    lg2_sm += [lg2_pc_it]

  round_sm = len(r1bit)

  print(    'module mod_5 ( clk, addr, Out ) ;', file=fp)
  print(    '', file=fp)
  print(    '  input clk;',file=fp)
  print(    '  input      [{:<2d}: 0] addr;'.format(lg2_pc-1), file=fp)
  print(    '  output reg [2 : 0] Out;', file=fp)
  print(    '', file=fp)
  for idx0 in range(round_sm):
    print(  '  wire       [{:<2d}: 0] r1_{:d};'.format(lg2_r1[idx0]-1, idx0), file=fp)
    print(  '  wire       [{:<2d}: 0] r2_{:d};'.format(lg2_r2[idx0]-1, idx0), file=fp)
    print(  '  wire       [{:<2d}: 0] r4_{:d};'.format(lg2_r4[idx0]-1, idx0), file=fp)
    print(  '  reg        [{:<2d}: 0] sm_{:d};'.format(lg2_sm[idx0]-1, idx0), file=fp)
  print(    '', file=fp)
  for idx0 in range(round_sm):
    print(  '  assign r1_{:d} = '.format(idx0), end='', file=fp)
    for idx1 in range(r1bit[idx0]):
      if(idx1 != 0):
        print(' + ', end='', file=fp)
      if(idx0 == 0):
        bitstr = 'addr[{}]'
      else:
        bitstr = 'sm_{}[{{}}]'.format(idx0-1)
      if(idx1 % 2 == 0):
        print(bitstr.format(2*idx1), end='', file=fp)
      else:
        print(bitstr.format(2*idx1+1), end='', file=fp)
    print(  ';', file=fp)
    print(  '  assign r2_{:d} = '.format(idx0), end='', file=fp)
    for idx1 in range(r2bit[idx0]):
      if(idx1 != 0):
        print(' + ', end='', file=fp)
      if(idx0 == 0):
        bitstr = 'addr[{}]'
      ...
      print(bitstr.format(2*idx1+1), end='', file=fp)
    print(  ';', file=fp)
    print(  '  assign r4_{:d} = '.format(idx0), end='', file=fp)
    for idx1 in range(r4bit[idx0]):
      if(idx1 != 0):
        print(' + ', end='', file=fp)
      if(idx0 == 0):
        bitstr = 'addr[{}]'
        ...
        bitstr = 'sm_{}[{{}}]'.format(idx0-1)
      print(bitstr.format(4*idx1+2), end='', file=fp)
    print(  ';', file=fp)
    print(  '', file=fp)
    print(  '  always @ ( posedge clk ) begin', file=fp)
    print(  '    sm_{idx} <= {{r4_{idx}, 2\'b0}} + {{r2_{idx}, 1\'b0}} + r1_{idx};'.format(idx=idx0), file=fp)
    print(  '  end', file=fp)
    print(  '', file=fp)
  print(    '  always @ ( posedge clk ) begin', file=fp)
  print(    '    case(sm_{})'.format(round_sm-1), file=fp)
  for idx0 in range(summax[round_sm-1]+1):
    print(  '      {loesm:d}\'d{idx:<2d}:   Out <= \'d{residue};'.format(loesm=lg2_sm[round_sm-1],idx=idx0,residue=idx0 % goodfactor), file=fp)
  print(    '      default: Out <= \'d0;', file=fp)
  print(    '    endcase', file=fp)
  print(    '  end', file=fp)
  print(    '', file=fp)
  ...
  print(    'module good5_addr_gen ( clk, y_deg, in_good, out_good0, out_good1, acc_ctrl );', file=fp)
  print(    '', file=fp)
  print(    '  input            clk;',file=fp)
  print(    '  input      [2:0] y_deg;',file=fp)
  print(    '  input      [2:0] in_good;', file=fp)
  print(    '  output reg [2:0] out_good0;', file=fp)
  print(    '  output reg [2:0] out_good1;', file=fp)
  print(    '  output reg       acc_ctrl;', file=fp)
  print(    '', file=fp)
  print(    '  wire             in_good_reset;',file=fp)
  print(    '  assign in_good_reset = (in_good == 3\'d0);', file=fp)
  print(    '', file=fp)
  print(    '  always @ ( posedge clk ) begin', file=fp)
  print(    '    out_good0 <= in_good;', file=fp)
  print(    '    case({ y_deg, in_good })', file=fp)
  for idx0 in range(goodfactor):
    for idx1 in range(goodfactor):
      print('      {{ 3\'d{}, 3\'d{} }}: out_good1 <= 3\'d{};'.format(idx0, idx1, (idx0 - idx1) % goodfactor), file=fp)
  print(    '      default:        out_good1 <= 3\'d0;', file=fp)
  print(    '    endcase', file=fp)
  print(    '  end', file=fp)
  print(    '', file=fp)
  print(    '  always @ ( posedge clk ) begin', file=fp)
...
def bfu_gen(fp, qi, lg2_qi, lg2_2qi, lg2_wmax):
  ...
  print(  '  wire signed       [{:<2d} : 0] a_add_q, a_sub_q, b_add_q, b_sub_q;'.format(lg2_qi+2), file=fp)
  print(  '', file=fp)
  ...
  if(goodfactor == 3):
    ...
    print('  assign a_mux = (!acc)        ? in_a_s4 :',file=fp)
    print('                  a_add_q[{:<2d}]  ? a_add_q :'.format(lg2_qi+2), file=fp)
    print('                 (!a_sub_q[{:<2d}]) ? a_sub_q : a;'.format(lg2_qi+2), file=fp)
  print(  '', file=fp)
  print(  '  always @(posedge clk ) begin', file=fp)
...
def NTT_RTLGen():
  ...
  if(goodfactor == 3):
    mod_delay = mod3_gen(fp, lg2_pc)
  elif(goodfactor == 5):
    mod_delay = mod5_gen(fp, lg2_pc)
  ntt_top_gen(fp, mod_delay)
